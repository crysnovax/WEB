  ...
  async fetch(request, env, ctx) {
    ...
    async function sendNotification(subject, body) {
      ...
      try {
        ...
        await fetch('https://api.resend.com/emails', { method: 'POST', headers: { 'Authorization': 'Bearer ' + env.RESEND_API_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: 'CRYSNOVA LIVE <notify@web.crysnovax.link>', to: emails, subject, html: body }) });
      } catch (e) {}
    }
...
function scripts(env) {
  return `
    ...
    function loginWithGitHub(){var w=600,h=600;var l=(screen.width-w)/2,t=(screen.height-h)/2;window.open("https://github.com/login/oauth/authorize?client_id="+GITHUB_CLIENT_ID+"&redirect_uri="+encodeURIComponent("https://web.crysnovax.link/auth/github/callback")+"&scope=read:user","GitHub","width="+w+",height="+h+",left="+l+",top="+t)}
