 ...
 //     const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${plugin.name} - CRYSNOVA</title><style>body{background:#0a0f1e;color:#e0f2fe;font-family:Inter;padding:2rem}pre{background:#0d1528;padding:1rem;border-radius:8px;overflow:auto}</style></head><body><h1>${plugin.name}</h1><p>${plugin.description}</p><p>👤 ${plugin.authorName || plugin.author} | ⬇️ ${downloads} | ⭐ ${avgRating} (${cnt})</p><h3>Installation</h3><code>.plugin ${plugin.customUrl || plugin.url}</code><h3>Source Code</h3><pre>${escapeHtml(plugin.code)}</pre><p><a href="/plugins">← Back to Plugins</a></p></body></html>`;
  ...
  //  }
   // function escapeHtml(text) { return text.replace(/[&<>]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' })[c]); }
    ...
    htmlParts.push('<h1>' + (pageTitle === 'CRYSN⚉VA LIVE 🜲' ? '❄️ CRYSN⚉VA LIVE 🜲' : (pageTitle === 'CRYSNⓘVA DEPLOY' ? '❄️ CRYSNⓘVA DEPLOY' : (pageTitle === 'CRYSN⎔VA PLUGINS' ? '🔌 CRYSN⎔VA PLUGINS' : (pageTitle === 'CRYS❂NVA ADMIN 𓊈𝑽꯭𝑰꯭𝑷ࠡࠡࠡࠡࠢ𓊉' ? '🛠️ CRYS❂NVA ADMIN' : '📤 CRYSN☉VA SUBMIT ✐')))) + '</h1>');
    htmlParts.push('<div class="subtitle">' + (pageTitle === 'CRYSN⚉VA LIVE 🜲' ? 'The AI Powerhouse Behind the Bot' : (pageTitle === 'CRYSNⓘVA DEPLOY' ? 'One‑Click Deployment Script' : (pageTitle === 'CRYSN⎔VA PLUGINS' ? 'Extend Your Bot with Plugins' : (pageTitle === 'CRYS❂NVA ADMIN 𓊈𝑽꯭𝑰꯭𝑷ࠡࠡࠡࠡࠢ𓊉' ? 'Review & Manage Submissions' : 'Share Your Plugin with the Community')))) + '</div>');
    htmlParts.push('</div>');
    ...
    htmlParts.push('async function ratePlugin(pluginId){if(!githubAccessToken){alert("Login with GitHub first");return;}const rating=prompt("Rate 1-5 stars:");if(!rating||isNaN(rating)||rating<1||rating>5){alert("Invalid rating");return;}try{const r=await fetch("/api/ratings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({pluginId,rating:parseInt(rating),accessToken:githubAccessToken})});const d=await r.json();if(d.success){alert("Rated! Average: "+d.average);loadPlugins();}else{alert("Rating failed: "+d.error);}}catch(e){alert("Error: "+e.message);}}');
    ...
    htmlParts.push('function escapeJs(s){if(!s)return"";return String(s).replace(/\\\\/g,"\\\\\\\\").replace(/\'/g,"\\\\\'").replace(/\\n/g,"\\\\n").replace(/\\r/g,"\\\\r");}');
